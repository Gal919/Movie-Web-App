...
const Home = () => {
  ...
  const { data, isLoading } = useFetch(
    `https://www.omdbapi.com/?s=${value}&apikey=a165f90d`,
    value
  );
  ...
  const removeMovieFromFavorites = (movie) => {
    setFavoriteMovies(
      favoriteMovies.filter((favourite) => favourite.imdbID !== movie.imdbID)
    );
  };
  ...
  return (
    <div className="home">
        ...
        data?.data?.Search?.map((movie) => (
          <div key={movie.imdbID}>
            ...
            {favoriteMovies.find(
              ...
              <AddToFavorites onAddMovie={addMovieToFavorites} data={movie} />
            )}
          </div>
        ))
      )}
      {isLoading === false &&
        searchValue &&
        data?.data?.Response === "False" && <p>Movie not found</p>}
    </div>
  );
};
